...
export default function DataCard({card,selected,onSelect,refresh,busy}:{refresh?:()=>void;busy?:boolean;card:UnitCard;selected?:UnitCard["weapons"][number];onSelect?:(weapon:UnitCard["weapons"][number])=>void}){return <article className="datasheet" style={factionStyle(card.faction)}>
      ...
      <CardSource date={card.source.retrievedAt} url={card.source.url} edition={card.source.edition} warning={card.warning} refresh={refresh} busy={busy} photo={card.image}/>
