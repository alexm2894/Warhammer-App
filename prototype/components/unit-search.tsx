...
export default function UnitSearch({catalogue,onCard,onSearching,recentScope,roster=[],rosterLabel='Selected army',disabled=false}:Props){
 ...
 return <div className="unit-search">
  ...
  <div className="candidates">{choices.map(unit=><button key={unit.id} disabled={busy||listening||disabled} style={factionStyle(unit.faction)} className="faction-tile" onClick={()=>void open(unit)}><UnitImage unit={unit}/><strong>{unit.name}</strong><span>{unit.faction}</span></button>)}</div>
 </div>;
}
