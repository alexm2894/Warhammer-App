...
export default function Lookup(){
 ...
 <div hidden={collapsed}><details className="source-details"><summary>Session & source</summary><button className="action" onClick={()=>{clearSessionCards();setCard(null);setOpened([]);setSelectedRule('');}}>Clear unit cache</button>{card&&<><p>{card.source.edition}th edition · Retrieved {new Date(card.source.retrievedAt).toLocaleDateString()}</p><a href={card.source.url} target="_blank" rel="noreferrer">View on Wahapedia</a><button className="action" onClick={refresh}>Refresh unit card</button>{card.image&&<p><a href={card.image.source} target="_blank" rel="noreferrer">Photo: {card.image.credit}{card.image.caption&&<span> · {card.image.caption}</span>}</a></p>}</>}</details><p role="status">{message}</p></div></aside>{rule?<RuleCard key={rule.key} card={rule} refresh={rules.refresh} busy={rules.loading}/>:card?<DataCard key={card.id} card={card}/>:<section className="empty-card"><h2>Your cards, at a glance</h2><p>Speak or type a unit name, or choose an army for its rules.</p></section>}</div></AppShell>;
}
