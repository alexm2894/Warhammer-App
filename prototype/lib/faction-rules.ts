...
export interface RuleSection {id:string; name:string; text:string; url:string; options?:RuleSection[];kind?:'stratagem'|'enhancement';cp?:string;category?:string;timing?:'your'|'enemy'|'either'}
...
export function parseFactionRules(html:string,faction:string,url:string):FactionRules {
 ...
 function walk(node: Node){
   ...
   if(current&&element.classList.contains('str11Wrap')){
    ...
    if(title?.textContent&&body?.textContent)(current.options??=[]).push({kind:'stratagem',cp:element.querySelector('.str11CP')?.textContent?.trim(),category:element.querySelector('.str11Type')?.textContent?.trim(),timing:element.querySelector('.str11ColorYour')?'your':element.querySelector('.str11ColorEnemy')?'enemy':element.querySelector('.str11ColorEither')?'either':undefined,id:title.id,name:`${title.textContent.trim()} · ${element.querySelector('.str11CP')?.textContent?.trim()||'Stratagem'}`,text:body.textContent.trim(),url:url+'#'+title.id});
   }
 ...
 for(let i=groups.indexOf(army)+1;i<firstDetachment-1;i++){if(/^Stratagems/.test(groups[i].id)&&groups[i].options?.length)(army.options??=[]).push(...groups[i].options!);else army.text+='\n'+groups[i].name+'\n'+groups[i].text;}
 if(!army.text.trim())throw Error('Army rules were empty.');
 const out=(g:typeof groups[number],name=g.name):RuleSection=>({id:g.id,name,options:g.options,text:g.text.replace(/[ \t]+/g,' ').replace(/\n\s*\n/g,'\n').trim(),url:url+'#'+g.id});
