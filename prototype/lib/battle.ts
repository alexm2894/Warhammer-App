...
import {activeAbilities,criticalWound,abilityDescription} from './weapon-abilities.ts';
...
export interface Conditions {cover:boolean;plunging:boolean;heavy:boolean;charged:boolean;isolated:boolean;katah:'none'|'sustained'|'lethal';hit:number;wound:number;ap:number;save:number;reviewed:boolean;abilityOverrides?:Record<string,boolean>;halfRange?:boolean;targetModels?:number;singleTarget?:boolean;strength?:number;toughness?:number;attacks?:number;damage?:number;criticalHit?:number;criticalWound?:number}
...
export function resolveAttack(a:Combatant,d:Combatant,w:Weapon,c:Conditions,p:Player,rules?:FactionRules){
 const notes:string[]=[],keywords=activeAbilities(w.keywords,d.card.keywords,c.abilityOverrides),ranged=w.kind==='ranged',has=(word:string)=>keywords.some(k=>k.toLowerCase().includes(word));
 ...
 let skill=Number.parseInt(w.skill),strength=Number.parseInt(w.strength);const toughness=Math.max(1,stat(d,'T')+(c.toughness||0)),save=stat(d,'SV');
 ...
 strength=Math.max(1,strength+(c.strength||0));
 ...
 const critical=c.criticalWound?clamp(c.criticalWound,2,6):criticalWound(keywords),criticalHit=clamp(c.criticalHit||6,2,6);
 const hit=has('torrent')?'Automatic':Math.min(criticalHit,clamp(skill-hitMod,2,6))+'+',wound=Math.min(critical,clamp(woundTarget(strength,toughness)-woundMod,2,6))+'+';
 ...
 const bonus=(name:string)=>Math.max(0,...keywords.map(k=>Number(k.match(new RegExp('^'+name+'\\s+(\\d+)','i'))?.[1]||0)));
 const groups=Math.floor(Math.max(0,c.targetModels||0)/5);
 const attackBonus=(c.attacks||0)+(c.halfRange?bonus('RAPID FIRE'):0)+groups*(has('blast')?Math.max(1,bonus('BLAST')):0)+groups*(c.singleTarget?bonus('CLEAVE'):0);
 const damageBonus=(c.damage||0)+(c.halfRange?bonus('MELTA'):0);
 const add=(value:string,n:number)=>!n?value:/^\d+$/.test(value)?String(Math.max(1,Number(value)+n)):`${value}${n>=0?'+':''}${n} (minimum 1)`;
 for(const keyword of new Set(keywords))notes.push(`${keyword}: ${abilityDescription(keyword,critical,criticalHit)}`);
 if(c.cover&&has('ignores cover'))notes.push('Selected cover has no effect because this weapon ignores cover.');
 return {hit,wound,criticalHit,criticalWound:critical,armour:armour>6?'No normal save':armour+'+',invulnerable:Number.isFinite(invul)?invul+'+':'Not applicable',recommended:best>6?'No save':Math.max(2,best)+'+',strength,toughness,ap,hitMod,woundMod,keywords:[...new Set(keywords)],notes,attacks:add(w.attacks,attackBonus),damage:add(w.damage,damageBonus)};
}
